...
class TransferAccommodation(models.TransientModel):
    ...
    _description = "Transferir Alojamiento"

    hospitalization_id = fields.Many2one('tgr.hospitalization', 'Hospitalización', required=True)
    patient_id = fields.Many2one ('hms.patient','Paciente', required=True)
    current_ward = fields.Many2one ('hospital.ward', 'Sala/Habitación Actual')
    current_bed = fields.Many2one ('hospital.bed', 'Cama Actual No.')
    new_ward = fields.Many2one ('hospital.ward', 'Sala/Habitación')
    new_bed = fields.Many2one ('hospital.bed', 'Cama No.')

    @api.model
    def default_get(self,fields):
        context = self._context or {}
        res = super(TransferAccommodation, self).default_get(fields)
        hospitalization = self.env['tgr.hospitalization'].browse(context.get('active_ids', []))
        res.update({
