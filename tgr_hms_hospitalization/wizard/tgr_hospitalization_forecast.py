...
class AcsHospitalizationForecast(models.TransientModel):
    _name = "tgr.hospitalization.forecast"
    _description = 'Pronóstico de Hospitalización'
    ...
    def get_tgr_final_due(self):
        for rec in self:
            rec.tgr_final_due = rec.tgr_amount_due + rec.amount_total

    hospitalization_id = fields.Many2one("tgr.hospitalization", string="Hospitalización", readonly=True, required=True)
    patient_id = fields.Many2one("hms.patient", related="hospitalization_id.patient_id", string="Paciente", readonly=True, required=True)
    partner_id = fields.Many2one("res.partner", related="hospitalization_id.patient_id.partner_id", string="Contacto", readonly=True, required=True)
    date = fields.Datetime("Fecha", default=fields.Datetime.now())
    line_ids = fields.One2many("tgr.hospitalization.forecast.line", 'order_id', string='Líneas de Pronóstico', readonly=False, copy=True)
    ...
    currency_id = fields.Many2one("res.currency", related='company_id.currency_id', string="Moneda", readonly=True, required=True)
    pricelist_id = fields.Many2one('product.pricelist', string='Tarifa', check_company=True, 
        domain="['|', ('company_id', '=', False), ('company_id', '=', company_id)]",
        help="Si cambia la tarifa, solo las líneas nuevas se verán afectadas.")
    total_invoiced = fields.Monetary(related="patient_id.total_invoiced", string="Total Facturado")
    tgr_amount_due = fields.Monetary(related="patient_id.tgr_amount_due", string="Monto Adeudado")
    tgr_final_due = fields.Monetary(compute=get_tgr_final_due, string="Monto Total Adeudado")
    ...
    def onchange_hospitalization(self):
        ForecastLine = self.env['tgr.hospitalization.forecast.line']
        if self.hospitalization_id:
            self.pricelist_id = self.hospitalization_id.pricelist_id.id
            ...
            lines = []
            data = hospitalization.tgr_hospitalization_invoicing()
            sequence = 0
            for product_data in data:
                ...
                if product:
                    tgr_pricelist_id = self.pricelist_id.id or self.env.context.get('tgr_pricelist_id')
                    if not product_data.get('price_unit') and (self.partner_id.property_product_pricelist or tgr_pricelist_id):
                        if tgr_pricelist_id:
                            pricelist_id = self.env['product.pricelist'].browse(tgr_pricelist_id)
                        else:
...
class AcsHospitalizationForecastLine(models.TransientModel):
    _name = 'tgr.hospitalization.forecast.line'
    _description = "Línea de Pronóstico"
    _order = "sequence, id"
    ...
    order_id = fields.Many2one('tgr.hospitalization.forecast', string='Orden', required=True, ondelete='cascade', index=True, copy=False)
    name = fields.Text(string='Descripción', required=True)
    sequence = fields.Integer(string='Secuencia', default=10)
    product_id = fields.Many2one('product.product', string='Producto', domain=[('sale_ok', '=', True)], change_default=True, ondelete='restrict')
    ...
    tax_ids = fields.Many2many('account.tax', string='Impuestos', domain=['|', ('active', '=', False), ('active', '=', True)])

    price_subtotal = fields.Monetary(compute='_compute_amount', string='Subtotal', readonly=True, store=True)
    price_tax = fields.Float(compute='_compute_amount', string='Monto de Impuestos', readonly=True, store=True)
    price_total = fields.Monetary(compute='_compute_amount', string='Total', readonly=True, store=True)
    currency_id = fields.Many2one(related='order_id.company_id.currency_id', store=True, string='Moneda', readonly=True)
    display_type = fields.Selection([
        ('line_section', "Sección")], help="Technical field for UX purpose.")
