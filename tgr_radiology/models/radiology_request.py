...
class PatientLabTestLine(models.Model):
    ...
    def _tgr_is_manager(self):
        is_manager = self.env.user.has_group("tgr_radiology.group_hms_radiology_manager")
        for rec in self:
            rec.is_manager = is_manager

    test_id = fields.Many2one("tgr.radiology.test", string="Test", ondelete="cascade", required=True)
    tgr_tat = fields.Char(related="test_id.tgr_tat", string="Turnaround Time", readonly=True)
    instruction = fields.Char(string="Special Instructions")
    radiology_request_id = fields.Many2one("tgr.radiology.request", string="Lines", ondelete="cascade")
    sale_price = fields.Float(string="Sale Price")
    company_id = fields.Many2one(
        "res.company", ondelete="restrict", string="Company", related="radiology_request_id.company_id"
    )
    ...
    patient_radiology_ids = fields.Many2many(
        "patient.radiology.test",
        "radiology_request_line_test_rel",
        "req_line_id",
        "patient_radiology_test_id",
        "Radiology Tests",
        ondelete="cascade",
    )
    ...
    def onchange_test(self):
        if self.test_id:
            ...
            if self.radiology_request_id.pricelist_id:
                ...
                price = self.radiology_request_id.patient_id.property_product_pricelist._get_product_price(
...
class RadiologyRequest(models.Model):
    ...
    def get_patient_age(self):
        for rec in self:
            ...
            if rec.patient_id.birthday:
                ...
                if delta.years <= 2:
                    age = (
                        str(delta.years) + _(" Year") + str(delta.months) + _(" Month ") + str(delta.days) + _(" Days")
                    )
                else:
    ...
    patient_age = fields.Char(
        compute="get_patient_age", string="Age", store=True, help="Computed patient age at the moment of the evaluation"
    )
    ...
    line_ids = fields.One2many(
        "radiology.request.line", "radiology_request_id", string="Radiology Test Line", copy=True
    )
    ...
    tgr_radiology_invoice_policy = fields.Selection(related="company_id.tgr_radiology_invoice_policy")
    ...
    def get_tgr_show_create_invoice(self):
        for rec in self:
            ...
            if not rec.invoice_id:
                ...
                elif rec.tgr_radiology_invoice_policy == "any_time" and not rec.invoice_exempt:
                    tgr_show_create_invoice = True
                elif rec.tgr_radiology_invoice_policy == "in_advance" and not rec.invoice_exempt:
                    tgr_show_create_invoice = True
    ...
    def button_requested(self):
        ...
        self.name = self.env["ir.sequence"].next_by_code("tgr.radiology.request")
    ...
    def button_accept(self):
        ...
        if company_id.tgr_radiology_invoice_policy == "in_advance":
            if not self.invoice_id:
                raise UserError(_("Invoice is not created yet."))
            elif (
                self.invoice_id
                and company_id.tgr_check_radiology_payment
    ...
    def create_invoice(self):
        ...
        if self.physician_id:
            tgr_context.update({"commission_partner_ids": self.physician_id.partner_id.id})
    ...
    def action_view_test_results(self):
        action = self.env["ir.actions.actions"]._for_xml_id("tgr_radiology.action_radiology_result")
    ...
    def tgr_common_invoice_radiology_data(self, invoice_id=False):
        data = []
        if self.ids:
