...
class AccountInvoice(models.Model):
    ...
    radiology_request_id = fields.Many2one(
        "tgr.radiology.request", string="Radiology Request", copy=False, ondelete="restrict"
    )
...
class TGRConsumableLine(models.Model):
    ...
    patient_radiology_test_id = fields.Many2one(
        "patient.radiology.test", string="Patient Radiology Test", ondelete="restrict"
    )
    radiology_test_id = fields.Many2one("tgr.radiology.test", string="Radiology Test", ondelete="restrict")
...
class TGRPatient(models.Model):
    ...
    radiology_request_ids = fields.One2many("tgr.radiology.request", "patient_id", string="Radiology Requests")
    ...
    def action_radiology_requests(self):
        action = self.env["ir.actions.actions"]._for_xml_id("tgr_radiology.hms_action_radiology_request")
...
class Physician(models.Model):
    ...
    def _tgr_rec_radiology_count(self):
        Labrequest = self.env["tgr.radiology.request"]
    ...
    radiology_request_count = fields.Integer(compute="_tgr_rec_radiology_count", string="# Radiology Request")
    radiology_result_count = fields.Integer(compute="_tgr_rec_radiology_count", string="# Radiology Result")
...
class IrAttachment(models.Model):
    ...
    def _tgr_portal_ensure_token(self):
