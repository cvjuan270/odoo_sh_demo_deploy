...
class PatientLabTest(models.Model):
    ...
    def _get_disclaimer(self):
        return self.env.user.sudo().company_id.tgr_radiology_disclaimer or ""

    name = fields.Char(
        string="Test ID", help="Radiology result ID", readonly=True, copy=False, index=True, tracking=True
    )
    test_id = fields.Many2one("tgr.radiology.test", string="Test", required=True, ondelete="restrict", tracking=True)
    ...
    physician_id = fields.Many2one(
        "hms.physician", string="Prescribing Doctor", help="Doctor who requested the test", ondelete="restrict"
    )
    radiology_physician_id = fields.Many2one(
        "hms.physician", string="Radiology Doctor", help="Doctor who Approved the test", ondelete="restrict"
    )
    ...
    radiology_request_id = fields.Many2one("tgr.radiology.request", string="Test Request", ondelete="restrict")
    report = fields.Text(string="Test Report")
    note = fields.Text(string="Extra Info")
    company_id = fields.Many2one(
        "res.company", ondelete="restrict", string="Company", default=lambda self: self.env.company
    )
    ...
    consumable_line_ids = fields.One2many("hms.consumable.line", "patient_radiology_test_id", string="Consumable Line")
    ...
    _sql_constraints = [("name_company_uniq", "unique (name,company_id)", "Test Name must be unique per company !")]

    def _track_subtype(self, init_values):
        self.ensure_one()
        if "state" in init_values and self.state == "done":
            return self.env.ref("tgr_radiology.mt_radiology_test_done")
    ...
    def _subscribe_physician(self):
        done_subtype = self.env.ref("tgr_radiology.mt_radiology_test_done").id
        ...
        for rec in self:
            if rec.physician_id.partner_id and rec.physician_id.partner_id.id not in rec.message_partner_ids.ids:
                rec.message_subscribe(
                    partner_ids=[rec.physician_id.partner_id.id], subtype_ids=[done_subtype, comment_subtype]
                )
    ...
    def action_radiology_test_send(self):
        ...
        template_id = self.env["ir.model.data"]._xmlid_to_res_id(
            "tgr_radiology.tgr_radiology_test_email", raise_if_not_found=False
        )
    ...
    def tgr_get_consume_locations(self):
        if not self.company_id.tgr_radiology_usage_location_id:
            raise UserError(
                _("Please define a location where the consumables will be used during the Radiology test in company.")
            )
        if not self.company_id.tgr_radiology_stock_location_id:
            raise UserError(_("Please define a Radiology location from where the consumables will be taken."))

        dest_location_id = self.company_id.tgr_radiology_usage_location_id.id
        source_location_id = self.company_id.tgr_radiology_stock_location_id.id
