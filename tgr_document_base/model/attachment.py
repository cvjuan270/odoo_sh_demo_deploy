...
class Attachment(models.Model):
    ...
    tag_ids = fields.Many2many(
        "tgr.document.tag",
        "tgr_attachment_document_tag_rel",
        "document_id",
        "tag_id",
        string="Tags",
        help="Classify and analyze your Document",
    )
    user_ids = fields.Many2many("res.users", "attachment_user_rel", "user_id", "doc_id", string="Users")

    # TGR: Allow to read Documents. Only by passs to read is_document is added as if condition.
    @api.model
    def check(self, mode, values=None):
        ...
        if self:
            ...
            self._cr.execute(
                "SELECT res_model, res_id, create_uid, public, res_field, is_document FROM ir_attachment WHERE id IN %s",
                [tuple(self.ids)],
            )
            for res_model, res_id, create_uid, public, res_field, is_document in self._cr.fetchall():
                if public and mode == "read":
                    continue
                # TGR: THIS NEW CODE WAS ADDED
                if is_document:
                    continue
