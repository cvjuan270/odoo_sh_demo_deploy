...
class HrEmployee(models.Model):
    _name = 'hr.employee'
    _inherit = ['hr.employee','tgr.whatsapp.mixin']


class WhatsappAnnouncement(models.Model):
    _name = 'tgr.whatsapp.announcement'
    _inherit = ['tgr.whatsapp.mixin']
    ...
    template_id = fields.Many2one("tgr.whatsapp.template", "Template")
    ...
    def tgr_create_message(self, mobile, partner=False, template=False, res_model=False, res_id=False):
    ...
    def send_message(self):
        ...
        if self.announcement_type=='contacts':
            for partner in self.partner_ids:
                if partner.mobile:
                    self.tgr_create_message(partner.mobile, partner, template=template, res_model='res.partner', res_id=partner.id)
        else:
            ...
            for employee in employees:
                ...
                if mobile:
                    self.tgr_create_message(mobile, False, template=template, res_model='hr.employee', res_id=employee.id)
