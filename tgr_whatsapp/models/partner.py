...
class ResPartner(models.Model):
    _name = 'res.partner'
    _inherit = ['res.partner','tgr.whatsapp.mixin']

    def _count_whatsapp(self):
        Announcement = self.env['tgr.whatsapp.announcement']
        for rec in self:
    ...
    whatsapp_ids = fields.One2many('tgr.whatsapp.message', 'partner_id', string='whatsapp')
    ...
    tgr_otp_whatsapp = fields.Char(string="OTP Whatsapp", copy=False)
    ...
    def get_tgr_verify_otp_msg_whatsapp(self):
        company_id = self.sudo().company_id or self.env.user.sudo().company_id
        return company_id.tgr_waba_verify_otp_template_id

    def whatsapp_chat_history(self):
        if not self.mobile:
            raise UserError(_("No Mobile no linked with Record."))     
        return self.tgr_whatsapp_chat_history(self.partner_id, self.mobile)

    def action_send_otp_whatsapp(self):
        self.generateotp_whatsapp()
        for rec in self:
            template_id = rec.get_tgr_verify_otp_msg_whatsapp()
            if template_id:
                if rec.mobile:
    ...
    def action_verify_otp_whatsapp(self):
        if self.tgr_otp_whatsapp:
            pass
        else:
            raise UserError(_('Please enter OTP.'))

        if self.generated_otp_whatsapp == self.tgr_otp_whatsapp:
            self.verified_mobile_whatsapp = True
        else:
            raise UserError(_('The OTP you entered is invalid. Please enter the correct OTP.'))

    def action_tgr_whatsapp(self):
        action = self.env["ir.actions.actions"]._for_xml_id("tgr_whatsapp.action_tgr_whatsapp")
    ...
    def action_tgr_whatsapp_announcement(self):
        action = self.env["ir.actions.actions"]._for_xml_id("tgr_whatsapp.action_whatsapp_announcement")
    ...
    def whatsapp_chat_history(self):
        if not self.mobile:
            raise UserError(_("No Mobile no linked with Record."))     
        return self.tgr_whatsapp_chat_history(self, self.mobile)
