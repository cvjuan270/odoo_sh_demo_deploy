...
class AcsWhatsAppMessage(models.Model):
    ...
    template_id = fields.Many2one("tgr.whatsapp.template", string="Template")
    whatsapp_announcement_id = fields.Many2one("tgr.whatsapp.announcement", string="Announcement")
    ...
    def tgr_sanatize_mobile_number(self, values):
    ...
    def create(self, vals_list):
        for values in vals_list:
            ...
            values = self.tgr_sanatize_mobile_number(values)
    ...
    def write(self, vals):
        ...
        if 'mobile' in vals:
            vals = self.tgr_sanatize_mobile_number(vals)
...
class TGRwhatsappMixin(models.AbstractModel):
    _name = "tgr.whatsapp.mixin"
    _description = "WhatsApp Mixin"

    @api.model
    def tgr_eval_message(self, object, message_exp):
        message = ''
        try:
    ...
    def send_whatsapp(self, message, mobile, partner=False, template=False, res_model=False, res_id=False):
        ...
        record = self.env['tgr.whatsapp.message'].create({
            'message': message,
    ...
    def tgr_whatsapp_chat_history(self, partner, mobile):
        """Hook method to add logic in relatd module"""
        pass
