...
class ResPartner(models.Model):
    ...
    def _get_age(self):
        ...
        for rec in self:
            ...
            if rec.birthday:
                ...
                if today.strftime("%m") == rec.birthday.strftime(
                    "%m"
                ) and today.strftime("%d") == rec.birthday.strftime("%d"):
    ...
    is_patient = fields.Boolean(
        compute="_is_patient",
        search="_patient_search",
        string="Is Patient",
        help="Check if customer is linked with patient.",
    )
    tgr_amount_due = fields.Monetary(
        compute="_compute_tgr_amount_due", currency_field="currency_id"
    )
    tgr_patient_id = fields.Many2one(
        "hms.patient", compute="_is_patient", string="Patient", readonly=True
    )

    def _compute_tgr_amount_due(self):
        MoveLine = self.env["account.move.line"]
        for record in self:
            amount_due = 0
            unreconciled_aml_ids = MoveLine.sudo().search(
                [
                    ("reconciled", "=", False),
