...
class TGRQrcodeMixin(models.AbstractModel):
    _name = "tgr.qrcode.mixin"
    _description = "QrCode Mixin"

    unique_code = fields.Char("Unique UID")
    qr_image = fields.Binary("QR Code", compute="tgr_generate_qrcode")

    def tgr_generate_qrcode(self):
...
class TGRHmsMixin(models.AbstractModel):
    _name = "tgr.hms.mixin"
    _description = "HMS Mixin"

    def tgr_prepare_invocie_data(self, partner, patient, product_data, inv_data):
        ...
        data = {
            ...
            "invoice_line_ids": self.tgr_get_invoice_lines(
                product_data, partner, inv_data, fiscal_position_id
            ),
        ...
        if Module.search(
            [("name", "=", "tgr_hms_commission"), ("state", "=", "installed")]
        ) and self.env.context.get("commission_partner_ids", False):
            data["commission_partner_ids"] = [
                (6, 0, [self.env.context.get("commission_partner_ids")])
            ]
        return data

    @api.model
    def tgr_create_invoice(self, partner, patient=False, product_data=[], inv_data={}):
        inv_data = self.tgr_prepare_invocie_data(
            partner, patient, product_data, inv_data
        )
    ...
    @api.model
    def tgr_get_invoice_lines(
        self, product_data, partner, inv_data, fiscal_position_id
    ):
        lines = []
        for data in product_data:
            ...
            if product:
                tgr_pricelist_id = self.env.context.get("tgr_pricelist_id")

                if not data.get("price_unit"):
                    price = product.with_context(
                        tgr_pricelist_id=tgr_pricelist_id
                    )._tgr_get_partner_price(quantity, uom_id, partner)
                    discount = product.with_context(
                        tgr_pricelist_id=tgr_pricelist_id
                    )._tgr_get_partner_price_discount(quantity, uom_id, partner)
                else:
                    price = data.get("price_unit", product.list_price)

                if inv_data.get("move_type", "out_invoice") in [
                    "out_invoice",
                    "out_refund",
                ]:
    ...
    @api.model
    def tgr_create_invoice_line(self, product_data, invoice):
        ...
        if product:
            ...
            if not product_data.get("price_unit"):
                price = product.with_context(
                    tgr_pricelist_id=tgr_pricelist_id
                )._tgr_get_partner_price(quantity, uom_id, invoice.partner_id)
                discount = product.with_context(
                    tgr_pricelist_id=tgr_pricelist_id
                )._tgr_get_partner_price_discount(quantity, uom_id, invoice.partner_id)
            else:
    ...
    def tgr_action_view_invoice(self, invoices):
        ...
        elif self.env.context.get("tgr_open_blank_list"):
    ...
    def tgr_apply_invoice_exemption(self):
        for rec in self:
            rec.invoice_exempt = False if rec.invoice_exempt else True


class TGRDocumntMixin(models.AbstractModel):
    _name = "tgr.document.mixin"
    ...
    def _tgr_attachemnt_count(self):
        for rec in self:
            attachments = rec._tgr_get_attachemnts()
            rec.attach_count = len(attachments)
            rec.attachment_ids = [(6, 0, attachments.ids)]

    attach_count = fields.Integer(
        compute="_tgr_attachemnt_count", readonly=True, string="Documents"
    )
    attachment_ids = fields.Many2many(
        "ir.attachment",
        "attachment_tgr_hms_rel",
        "record_id",
        "attachment_id",
        compute="_tgr_attachemnt_count",
        string="Attachments",
    )
