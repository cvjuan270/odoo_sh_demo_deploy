...
class TGRPatient(models.Model):
    ...
    gov_code_label = fields.Char(compute="tgr_get_gov_code_label", string="Government Identity Label")
    ...
    is_corpo_tieup = fields.Boolean(
        string="Corporate Tie-Up",
        help="If not checked, these Corporate Tie-Up Group will not be visible at all.",
    )
    corpo_company_id = fields.Many2one(
        "res.partner",
        string="Corporate Company",
    ...
    tgr_tag_ids = fields.Many2many(
        "hms.patient.tag",
        "patient_tag_hms_rel",
        "tag_id",
        "patient_tag_id",
        string="Etiquetas de Paciente",
    )

    invoice_count = fields.Integer(compute="_rec_count", string="# Invoices")
    occupation = fields.Char("Occupation")
    tgr_religion_id = fields.Many2one("tgr.religion", string="Religion")
    ...
    def tgr_get_gov_code_label(self):
        for rec in self:
    ...
    def write(self, values):
        ...
        return super(TGRPatient, self).write(values)
    ...
    def view_invoices(self):
        ...
        action = self.with_context(tgr_open_blank_list=True).tgr_action_view_invoice(invoices)
    ...
    def send_birthday_email(self):
        wish_template_id = self.env.ref("tgr_hms_base.email_template_birthday_wish", raise_if_not_found=False)
    ...
    def _name_search(self, name, domain=None, operator="ilike", limit=None, order=None):
        ...
        return self._search(domain=domain, limit=limit, order=order)
        ...
        for patient in self:
            patient.display_name = (
                ...
                if not patient.name
                    ...
                    "{}{}".format(
                        patient.vat and "[%s] " % patient.vat or "",
                        patient.name,
                    )
                )
            )
