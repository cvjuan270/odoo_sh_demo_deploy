...
class ProductProduct(models.Model):
    ...
    def tgr_get_pricelist(self, partner):
    ...
    def tgr_get_pricelist_price(self, pricelist, quantity, uom_id):
        if pricelist:
            uom = False
            if uom_id:
    ...
    def _tgr_get_partner_price(self, quantity=1, uom_id=False, partner=False):
        pricelist_id = self.tgr_get_pricelist(partner)
        #if any pricelist pass price based on pricelist else default price.
        if pricelist_id and getattr(pricelist_id, 'discount_policy', 'with_discount') != 'without_discount':
            price = self.tgr_get_pricelist_price(pricelist_id, quantity, uom_id)
        else:
            price = self.list_price
        return price

    #Only One level pricelist price is computed.
    def _tgr_get_partner_price_discount(self, quantity=1, uom_id=False, partner=False):
        discount = 0
        base_price = self.list_price
        pricelist_id = self.tgr_get_pricelist(partner)
        #if any pricelist pass price based on pricelist else default price.
        if pricelist_id and getattr(pricelist_id, 'discount_policy', 'with_discount') == 'without_discount':
            pricelist_price = self.tgr_get_pricelist_price(pricelist_id, quantity, uom_id)
...
class StockProductionLot(models.Model):
    ...
    def _search_product_qty(self, operator, value):
        valid_record = []
        product_id = self._context.get('tgr_product_id',False)
        domain =[('product_id','=',product_id)]
        if self._context.get('tgr_all_products',False):
            domain =[]
