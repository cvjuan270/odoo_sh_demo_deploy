...
class TGRDocumntViewerMixin(models.AbstractModel):
    _name = "tgr.document.view.mixin"
    _description = "Document Viewer Mixin"

    def _get_document_preview_url(self):
        base_url = self.env["ir.config_parameter"].sudo().get_param("web.base.url")
        for rec in self:
            rec.document_preview_url = base_url + "/my/tgr/image/%s/%s" % (self._name, rec.id)

    document_preview_url = fields.Char(compute=_get_document_preview_url, string="Document Preview Link")

    def tgr_action_attachments_preview(self):
        ...
        self.ensure_one()
        return {"type": "ir.actions.act_url", "name": "Preview", "target": "new", "url": self.document_preview_url}
