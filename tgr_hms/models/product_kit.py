...
class TGRMedicamentGroup(models.Model):
    _name = 'tgr.product.kit'
    _order = 'sequence asc'
    ...
    tgr_kit_line_ids = fields.One2many('tgr.product.kit.line', 'tgr_kit_id', string='Kit lines')
    description = fields.Text("Description")


class TGRProductKitLine(models.Model):
    _name = 'tgr.product.kit.line'
    _order = 'sequence asc'
    ...
    def write(self, values):
        res = super(TGRProductKitLine, self).write(values)
        self.mapped('product_template_id').tgr_update_price_for_kit()
        return res
    ...
    def create(self, vals_list):
        ...
        for record in res:
            record.product_template_id.tgr_update_price_for_kit()
        return res

    def unlink(self):
        product_template_ids = self.mapped('product_template_id')
        res = super(TGRProductKitLine, self).unlink()
        product_template_ids.tgr_update_price_for_kit()
        return res
...
class ProductTemplate(models.Model):
    ...
    @api.depends('tgr_kit_line_ids','is_kit_product','tgr_kit_line_ids.total_price')
    def tgr_get_kit_amount_total(self):
        for rec in self:
            rec.kit_amount_total = sum(rec.tgr_kit_line_ids.mapped('total_price'))
            rec.kit_cost_total = sum(rec.tgr_kit_line_ids.mapped('total_standard_price'))
    ...
    tgr_kit_line_ids = fields.One2many('tgr.product.kit.line', 'product_template_id', string='Kit Components')
    kit_amount_total = fields.Float(compute='tgr_get_kit_amount_total', string="Kit Total")
    kit_cost_total = fields.Float(compute='tgr_get_kit_amount_total', string="Kit Cost Total")
    ...
    def tgr_update_price_for_kit(self):
        for rec in self:
