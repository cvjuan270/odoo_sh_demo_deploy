...
class TGRTreatment(models.Model):
    _name = 'hms.treatment'
    ...
    medical_alert_ids = fields.Many2many('tgr.medical.alert', 'treatment_medical_alert_rel','treatment_id', 'alert_id',
    ...
    patient_procedure_ids = fields.One2many('tgr.patient.procedure', 'treatment_id', 'Patient Procedures')
    ...
    def default_get(self, fields):
        res = super(TGRTreatment, self).default_get(fields)
    ...
    def unlink(self):
        ...
        return super(TGRTreatment, self).unlink()
    ...
    def create_invoice(self):
        ...
        invoice = self.with_context(tgr_context).tgr_create_invoice(partner=self.patient_id.partner_id, patient=self.patient_id, product_data=[{'product_id': product_id}], inv_data={'hospital_invoice_type': 'treatment'})
    ...
    def action_create_procedure_invoice(self):
        ...
        invoice = self.tgr_create_invoice(partner=self.patient_id.partner_id, patient=self.patient_id, product_data=product_data, inv_data=inv_data)
    ...
    def tgr_select_treatement_for_appointment(self):
        if self._context.get('tgr_current_appointment'):
            #Check if we can get back to appointment in breadcrumb.
            appointment = self.env['hms.appointment'].search([('id','=',self._context.get('tgr_current_appointment'))])
