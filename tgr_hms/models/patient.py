...
class TGRPatient(models.Model):
    ...
    def _rec_count(self):
        rec = super(TGRPatient, self)._rec_count()
    ...
    def _tgr_get_attachemnts(self):
        attachments = super(TGRPatient, self)._tgr_get_attachemnts()
    ...
    def _patient_evaluation_graph_data(self):
        for rec in self:
            ...
            records = self.env['tgr.patient.evaluation'].search(domain, order="date")
    ...
    def tgr_check_cancellation_flag(self):
        tgr_flag_days = self.env.user.sudo().company_id.tgr_flag_days or 365
        tgr_flag_count_limit = self.env.user.sudo().company_id.tgr_flag_count_limit or 1
        date_start = fields.Datetime.now() - relativedelta(days=tgr_flag_days)
        ...
        for rec in self:
            ...
            if cancelled_appointments >= tgr_flag_count_limit:
                show_cancellation_warning_flag = True
            rec.show_cancellation_warning_flag = show_cancellation_warning_flag
            rec.tgr_flag_days = tgr_flag_days
            rec.tgr_cancelled_appointments = cancelled_appointments
    ...
    ethnic_group_id = fields.Many2one('tgr.ethnicity', string='Ethnic group')
    cod_id = fields.Many2one('hms.diseases', string='Cause of Death')
    family_member_ids = fields.One2many('tgr.family.member', 'patient_id', string='Family')
    ...
    medical_alert_ids = fields.Many2many('tgr.medical.alert', 'patient_medical_alert_rel','patient_id', 'alert_id',
    ...
    evaluation_ids = fields.One2many('tgr.patient.evaluation', 'patient_id', 'Evaluations')

    last_evaluation_id = fields.Many2one("tgr.patient.evaluation", string="Last Evaluation", compute=_get_last_evaluation, readonly=True, store=True)
    ...
    tgr_weight_name = fields.Char(related="last_evaluation_id.tgr_weight_name", string='Patient Weight unit of measure label')
    tgr_height_name = fields.Char(related="last_evaluation_id.tgr_height_name", string='Patient Height unit of measure label')
    tgr_temp_name = fields.Char(related="last_evaluation_id.tgr_temp_name", string='Patient Temp unit of measure label')
    tgr_spo2_name = fields.Char(related="last_evaluation_id.tgr_spo2_name", string='Patient SpO2 unit of measure label')
    tgr_rbs_name = fields.Char(related="last_evaluation_id.tgr_rbs_name", string='Patient RBS unit of measure label')

    patient_procedure_ids = fields.One2many('tgr.patient.procedure', 'patient_id', 'Patient Procedures')
    patient_procedure_count = fields.Integer(compute='_rec_count', string='# Patient Procedures')
    show_cancellation_warning_flag = fields.Boolean(compute='tgr_check_cancellation_flag', string='Show Cancellation Flag')
    tgr_flag_days = fields.Integer(compute='tgr_check_cancellation_flag', string='Flag Days')
    tgr_cancelled_appointments = fields.Integer(compute='tgr_check_cancellation_flag', string='Cancelled Appointments')
    ...
    def show_weight_chart(self):
        action = self.env["ir.actions.actions"]._for_xml_id("tgr_hms.action_patient_evaluation_graph_1")
    ...
    def create_invoice(self):
        ...
        invoice = self.tgr_create_invoice(partner=self.partner_id, patient=self, product_data=[{'product_id': product_id}], inv_data={'hospital_invoice_type': 'patient'})
...
class TGRFamilyMember(models.Model):
    _name = 'tgr.family.member'
    ...
    relation_id = fields.Many2one('tgr.family.relation', string='Relation', required=True)
    inverse_relation_id = fields.Many2one("tgr.family.member", string="Inverse Relation")
    ...
    def unlink(self):
        ...
        res = super(TGRFamilyMember, self).unlink()
        if inverse_relation_id:
            inverse_relation_id.unlink()
        return res

    def write(self, values):
        res = super(TGRFamilyMember, self).write(values)
