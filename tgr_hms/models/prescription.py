...
class TGRPrescriptionOrder(models.Model):
    _name='prescription.order'
    _description = "Prescription Order"
    _inherit = ['mail.thread', 'mail.activity.mixin', 'tgr.hms.mixin', 'tgr.qrcode.mixin']
    _order = 'id desc'
    ...
    medical_alert_ids = fields.Many2many('tgr.medical.alert', 'prescription_medical_alert_rel','prescription_id', 'alert_id',
    ...
    def unlink(self):
        ...
        return super(TGRPrescriptionOrder, self).unlink()
    ...
    def print_report(self):
        return self.env.ref('tgr_hms.report_hms_prescription_id').report_action(self)
    ...
    def action_prescription_send(self):
        ...
        '''
        self.ensure_one()
        template_id = self.env['ir.model.data']._xmlid_to_res_id('tgr_hms.tgr_prescription_email', raise_if_not_found=False)
        ctx = {
...
class TGRPrescriptionLine(models.Model):
    ...
    def onchange_product(self):
        if self.product_id:
            ...
            self.allow_substitution = self.product_id.tgr_allow_substitution
