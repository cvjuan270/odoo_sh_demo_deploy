...
class Appointment(models.Model):
    ...
    _inherit = ['mail.thread', 'mail.activity.mixin', 'tgr.hms.mixin', 'tgr.document.mixin']
    _order = "id desc"
    ...
    def _tgr_get_invoice_count(self):
        AccountMove = self.env['account.move']
        for rec in self:
            rec.invoice_count = len(rec.invoice_ids)

    def _tgr_invoice_policy(self):
        for rec in self:
    ...
    def get_procudures_to_invoice(self):
        Procedure = self.env['tgr.patient.procedure']
        for rec in self:
            procedures = Procedure.search([('appointment_ids', 'in', rec.id), ('invoice_id','=', False)])
            rec.procedure_to_invoice_ids = [(6, 0, procedures.ids)]

    def tgr_get_department(self):
        for rec in self:
            tgr_department_id = False
            if rec.department_id and rec.department_id.id:
                tgr_department_id = self.env['hr.department'].sudo().search([('id','=',rec.department_id.id)]).id
            rec.tgr_department_id = tgr_department_id
    ...
    #TGR: Added department field agian here to avoid portal error. Insted of reading department_id used tgr_department_idfield so error vanbe avoided.
    tgr_department_id = fields.Many2one('hr.department', compute="tgr_get_department")
    ...
    tgr_reminder_sent = fields.Boolean("Reminder Sent", default=False)

    evaluation_ids = fields.One2many('tgr.patient.evaluation', 'appointment_id', 'Evaluations')
    evaluation_id = fields.Many2one('tgr.patient.evaluation', ondelete='restrict', compute=_get_evaluation,
    ...
    tgr_weight_name = fields.Char(related="evaluation_id.tgr_weight_name", string='Patient Weight unit of measure label')
    tgr_height_name = fields.Char(related="evaluation_id.tgr_height_name", string='Patient Height unit of measure label')
    tgr_temp_name = fields.Char(related="evaluation_id.tgr_temp_name", string='Patient Temp unit of measure label')
    tgr_spo2_name = fields.Char(related="evaluation_id.tgr_spo2_name", string='Patient SpO2 unit of measure label')
    tgr_rbs_name = fields.Char(related="evaluation_id.tgr_rbs_name", string='Patient RBS unit of measure label')
    ...
    appointment_invoice_policy = fields.Selection([('at_end','Invoice in the End'),
        ...
        ('advance','Invoice in Advance')],compute=_tgr_invoice_policy, string="Appointment Invoicing Policy")
    ...
    planned_duration = fields.Float('Duration', compute="_get_planned_duration", inverse='_inverse_planned_duration',
        default=lambda self: self.env.company.tgr_appointment_planned_duration)
    ...
    medical_alert_ids = fields.Many2many('tgr.medical.alert', 'appointment_medical_alert_rel','appointment_id', 'alert_id',
    ...
    cancel_reason_id = fields.Many2one('tgr.cancel.reason', string='Cancellation Reason')
    user_id = fields.Many2one('res.users',string='Responsible',
        ondelete='cascade', help='Responsible User for appointment validation And further Followup.')
    tgr_kit_id = fields.Many2one('tgr.product.kit', string='Kit')
    tgr_kit_qty = fields.Integer("Kit Qty", default=1)
    invoice_ids = fields.One2many("account.move", "appointment_id", string="Invoices", groups="account.group_account_invoice")
    invoice_count = fields.Integer(compute="_tgr_get_invoice_count", string="#Invoices", groups="account.group_account_invoice")
    procedure_to_invoice_ids = fields.Many2many('tgr.patient.procedure', 'tgr_appointment_procedure_rel', 'appointment_id', 'procedure_id', compute="get_procudures_to_invoice", string="Procedures to Invoice")
    ...
    #TGR NOTE: Because of error for portal appointment creation added _compute_field_value method.
    ...
    tgr_show_create_invoice = fields.Boolean(compute="get_tgr_show_create_invoice", string="Show Create Invoice Button")
    tgr_show_conumable_create_invoice = fields.Boolean(compute="get_tgr_show_create_invoice", string="Show Consumable Create Invoice Button")

    #TGR: Compute visiblity of create invocie button.
    def get_tgr_show_create_invoice(self):
        for rec in self:
            tgr_show_create_invoice = False
            if not rec.invoice_id :
                if rec.state=='to_invoice':
                    tgr_show_create_invoice = True
            ...
            rec.tgr_show_conumable_create_invoice = True if ((not rec.invoice_id) and 
                                                   rec.state!='done' and 
            ...
            rec.tgr_show_create_invoice = tgr_show_create_invoice
    ...
    def default_get(self, fields):
        ...
        if self._context.get('tgr_department_type'):
            department = self.env['hr.department'].search([('department_type','=',self._context.get('tgr_department_type'))], limit=1)
            if department:
    ...
    def update_reminder_dates(self):
        for rec in self:
            if fields.Datetime.now() < rec.date:
                reminder_date = rec.date - timedelta(days=int(rec.company_id.tgr_reminder_day)) - timedelta(hours=int(rec.company_id.tgr_reminder_hours))
    ...
    def send_appointment_reminder(self):
        ...
        reminder_appointments = self.sudo().search([('tgr_reminder_sent', '=', False),('state', 'in', ['draft','confirm']),('date','>',fields.Datetime.now()),('reminder_date','<=', date_time_now)])
        if reminder_appointments:
            for reminder_appointment in reminder_appointments:
                reminder_appointment.tgr_reminder_sent = True
    ...
    def print_report(self):
        return self.env.ref('tgr_hms.action_appointment_report').report_action(self)
    ...
    def action_appointment_send(self):
        ...
        '''
        self.ensure_one()
        template_id = self.env['ir.model.data']._xmlid_to_res_id('tgr_hms.tgr_appointment_email', raise_if_not_found=False)
    ...
    def tgr_appointment_inv_product_data(self, with_product=True):
        product_data = []
        if with_product:
        ...
        #TGR: Check if we need it or not as it is getting created in combined 
    ...
    def tgr_appointment_inv_data(self):
        return {
    ...
    def tgr_appointment_common_data(self, invoice_id):
        """
            ...
            1> Procedures: that are already coverd in tgr_appointment_inv_product_data and dene here also
        ...
        """
        #Procedure Invoicing
        data = self.procedure_to_invoice_ids.tgr_common_invoice_procedure_data(invoice_id)
        return data

    #mehtod to create invoice on related records like done in hospitalization
    def tgr_appointment_common_invoicing(self, invoice_id):
        data = self.tgr_appointment_common_data(invoice_id)
        #create Invoice lines only if invoice is passed
        if invoice_id:
            for line in data:
                inv_line = self.with_context(tgr_pricelist_id=self.pricelist_id.id).tgr_create_invoice_line(line, invoice_id)
        return data

    def create_invoice(self):
        inv_data = self.tgr_appointment_inv_data()
        product_data = self.tgr_appointment_inv_product_data()
        tgr_context = {'commission_partner_id':self.physician_id.partner_id.id}
        if self.pricelist_id:
            tgr_context.update({'tgr_pricelist_id': self.pricelist_id.id})
        invoice = self.with_context(tgr_context).tgr_create_invoice(partner=self.patient_id.partner_id, patient=self.patient_id, product_data=product_data, inv_data=inv_data)
        self.invoice_id = invoice.id
        self.tgr_appointment_common_invoicing(invoice)
        if self.state == 'to_invoice':
            self.appointment_done()

        if self.state == 'draft' and not self._context.get('avoid_confirmation'):
            if self.invoice_id and not self.company_id.tgr_check_appo_payment:
    ...
    def create_consumed_prod_invoice(self):
        ...
        inv_data = self.tgr_appointment_inv_data()
        product_data = self.tgr_appointment_inv_product_data(with_product=False)

        pricelist_context = {}
        if self.pricelist_id:
            pricelist_context = {'tgr_pricelist_id': self.pricelist_id.id}
        invoice = self.with_context(pricelist_context).tgr_create_invoice(partner=self.patient_id.partner_id, patient=self.patient_id, product_data=product_data, inv_data=inv_data)
    ...
    def view_invoice(self):
        ...
        action = self.tgr_action_view_invoice(appointment_invoices)
    ...
    def action_refer_doctor(self):
        action = self.env["ir.actions.actions"]._for_xml_id("tgr_hms.action_appointment")
        ...
        action['views'] = [(self.env.ref('tgr_hms.view_hms_appointment_form').id, 'form')]
        return action
    ...
    def onchange_physician(self):
        product_id = False
        #TGR: First check configuration on department.
        if self.tgr_department_id:
            #TGR: To avoid portal access error research department here.
            if self.consultation_type=='followup':
                if self.tgr_department_id.followup_service_id:
                    product_id = self.tgr_department_id.followup_service_id.id

            elif self.tgr_department_id.consultaion_service_id:
                product_id = self.tgr_department_id.consultaion_service_id.id
        ...
        if self.physician_id:
            ...
            if self.physician_id.appointment_duration and not self._context.get('tgr_online_transaction'):
    ...
    def appointment_confirm(self):
        if (not self._context.get('tgr_online_transaction')) and (not self.invoice_exempt):
            if self.appointment_invoice_policy=='advance' and not self.invoice_id:
                raise UserError(_('Invoice is not created yet'))

            elif self.invoice_id and self.company_id.tgr_check_appo_payment and self.payment_state not in ['in_payment','paid']:
                raise UserError(_('Invoice is not Paid yet.')) 

        if not self.user_id:
            self.user_id = self.env.user.id

        if self.patient_id.email and (self.company_id.tgr_auto_appo_confirmation_mail or self._context.get('tgr_online_transaction')):
            template = self.env.ref('tgr_hms.tgr_appointment_email')
    ...
    def action_view_treatment(self):
        action = self.env["ir.actions.actions"]._for_xml_id("tgr_hms.tgr_action_form_hospital_treatment")
        action['context'] = {
            'default_appointment_ids': [(6,0,self.ids)],
            'default_patient_id': self.patient_id.id,
            'tgr_current_appointment': self.id,
        }
        action['views'] = [(self.env.ref('tgr_hms.view_hospital_hms_treatment_form').id, 'form')]
        if self.treatment_id:
            ...
            action['views'] = [(self.env.ref('tgr_hms.view_tgr_hms_treatment_appointment_tree').id, 'tree')]
        return action
    ...
    def get_tgr_kit_lines(self):
        if not self.tgr_kit_id:
            raise UserError("Please Select Kit first.")

        lines = []
        for line in self.tgr_kit_id.tgr_kit_line_ids:
            lines.append((0,0,{
                'product_id': line.product_id.id,
                'product_uom_id': line.product_id.uom_id.id,
                'qty': line.product_qty * self.tgr_kit_qty,
            }))
    ...
    def _create_edit_followup_reminder(self):
        ...
        for rec in self:
            if rec.follow_date:
                ...
                activity_type = company.tgr_followup_activity_type_id or default_activity_type
    ...
    def cancel_old_appointments(self):
        ...
        if self.env.user.sudo().company_id.tgr_cancel_old_appointment:
    ...
    def tgr_reschedule_appointments(self, reschedule_time):
        for rec in self:
