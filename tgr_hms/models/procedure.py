...
class AcsPatientProcedure(models.Model):
    ...
    def _tgr_get_attachemnts(self):
        attachments = super(AcsPatientProcedure, self)._tgr_get_attachemnts()
    ...
    duration = fields.Float('Duration', compute="tgr_get_duration", store=True)

    diseas_id = fields.Many2one('hms.diseases', 'Disease')
    description = fields.Text(string="Description")
    treatment_id = fields.Many2one('hms.treatment', 'Treatment')
    appointment_ids = fields.Many2many('hms.appointment', 'tgr_appointment_procedure_rel', 'appointment_id', 'procedure_id', 'Appointments')
    ...
    def create(self, vals_list):
        for values in vals_list:
            values['name'] = self.env['ir.sequence'].next_by_code('tgr.patient.procedure') or 'New Procedure'
    ...
    def action_create_invoice(self):
        ...
        tgr_context = {'commission_partner_ids':self.physician_id.partner_id.id}
    ...
    def action_show_details(self):
        ...
        action['views'] = [(self.env.ref('tgr_hms.view_tgr_patient_procedure_form').id, 'form')]
        action['target'] = 'new'
        return action
    ...
    def tgr_common_invoice_procedure_data(self, invoice_id=False):
        data = []
        if self.ids:
...
class StockMove(models.Model):
    ...
    procedure_id = fields.Many2one('tgr.patient.procedure', ondelete="cascade", string="Procedure")
