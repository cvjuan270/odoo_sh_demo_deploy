...
class AccountInvoice(models.Model):
    ...
    request_id = fields.Many2one('tgr.laboratory.request', string='Lab Request', copy=False, ondelete='restrict')
...
class TGRConsumableLine(models.Model):
    ...
    lab_test_id = fields.Many2one('tgr.lab.test', string="Lab Test", ondelete="restrict")
...
class TGRPatient(models.Model):
    ...
    request_ids = fields.One2many('tgr.laboratory.request', 'patient_id', string='Lab Requests')
    ...
    def action_lab_requests(self):
        action = self.env["ir.actions.actions"]._for_xml_id("tgr_laboratory.hms_action_lab_test_request")
        action['domain'] = [('id','in',self.request_ids.ids)]
        action['context'] = {'default_patient_id': self.id}
        return action

    def action_view_lab_test_results(self):
        action = self.env["ir.actions.actions"]._for_xml_id("tgr_laboratory.action_lab_result")
...
class Physician(models.Model):
    ...
    def _tgr_lab_rec_count(self):
        Labrequest = self.env['tgr.laboratory.request']
    ...
    lab_request_count = fields.Integer(compute='_tgr_lab_rec_count', string='# Lab Request')
    lab_result_count = fields.Integer(compute='_tgr_lab_rec_count', string='# Lab Result')
...
class ResUsers(models.Model):
    ...
    default_collection_center_id = fields.Many2one('tgr.laboratory',  string='Collection Center')
